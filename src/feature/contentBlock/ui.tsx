...
export default function ContentPanel({
    slice, defaultLayout, className,
    editing, onOpenEdit, onCloseEdit,
    layoutOverride, onLayoutChange,
    uiFocused = false,
    uiSelected = false,
}: Props) {
    ...
    const openEditor = () => {
        ...
        setDraftLayout(base);
        if (onOpenEdit) onOpenEdit(); else setOpenEdit(true);
    };
    ...
    const handleSubmit = (data: SubmitImageTypeHeaderLayout) => {
        ...
        if (onCloseEdit) onCloseEdit(); else setOpenEdit(false);
    };
